    ...
    if (menuState == menuMain) {
      ...
      if (buttonOK.isHolded() && validKeyReceived) {
        if (saveKeyToSD(&keyData1)) {
          ...
          display.drawBitmap(16, 6, image_DolphinSaved_bits, 92, 58, SSD1306_WHITE);  // Изменено
          display.setTextColor(SSD1306_WHITE);  // Изменено
      ...
      if (buttonOK.isClick()) {
        ...
        if (SD.remove(filePath)) {
          display.clearDisplay();
          display.drawBitmap(5, 2, image_DolphinMafia_bits, 119, 62, SSD1306_WHITE);  // Изменено
          display.setTextColor(SSD1306_WHITE);  // Изменено
...
void OLED_printWaitingSignal() {
  display.clearDisplay();
  display.drawBitmap(0, 4, image_DolphinReceive_bits, 97, 61, SSD1306_WHITE);  // Изменено
  display.setTextColor(SSD1306_WHITE);  // Изменено
...
void OLED_printKey(tpKeyData* kd, String fileName, bool isSending) {
  ...
  if (isSending) {
    ...
    display.drawBitmap(109, 40, image_satellite_dish_bits, 15, 16, SSD1306_WHITE);  // Изменено
  }
  display.display();
}
...
void OLED_printFileExplorer() {
  ...
  for (int i = 0; i < itemsToShow; i++) {
    ...
    if (idx == fileIndex) {
      display.fillRect(0, y - 1, 128, 11, SSD1306_WHITE);  // Изменено
      display.setTextColor(SSD1306_BLACK);  // Изменено
    } else {
      display.setTextColor(SSD1306_WHITE);  // Изменено
    }
...
void sendSynthKey(tpKeyData* kd) {
  ...
  display.drawBitmap(15, 12, image_Connected_bits, 62, 31, SSD1306_WHITE);  // Изменено
  display.display();
  delay(1000);
  OLED_printKey(kd, selectedFile);
}
...
void OLED_printAnalyzer(bool signalReceived, float detectedFreq) {
  display.clearDisplay();
  display.drawBitmap(0, 7, image_Dolphin_MHz_bits, 108, 57, SSD1306_WHITE);  // Изменено
  display.drawBitmap(100, 6, image_MHz_1_bits, 25, 11, SSD1306_WHITE);  // Изменено
  display.setTextColor(SSD1306_WHITE);  // Изменено
  display.setTextWrap(false);
...
void OLED_printJammer() {
  display.clearDisplay();
  display.drawBitmap(0, 3, image_Dolphin_Send_bits, 97, 61, SSD1306_WHITE);  // Изменено
  display.setTextColor(SSD1306_WHITE);  // Изменено
  display.setTextWrap(false);
  display.setCursor(72, 13);
  display.print(String(frequency, 2) + "MHz");
