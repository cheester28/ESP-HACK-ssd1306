...
void displayBadKBScriptExec(Adafruit_SSD1306 &display, const String& filename, const std::vector<String>& logs, int logTop);
...
static bool _bb_runDuckyScript(const char* filename, std::vector<String>& logs, Adafruit_SSD1306 &display) {
  if (!gServer || !gInput) {
...
void drawBadKBExplorer(Adafruit_SSD1306 &display) {
  ...
  for (int i = 0; i < perPage; i++) {
    ...
    if (idx == badKBFileIndex) {
      display.fillRect(0, 24 + i * 10 - 1, display.width(), 10, SSD1306_WHITE);
...
void drawDeleteConfirm(Adafruit_SSD1306 &display, const String& filename) {
  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(SSD1306_WHITE);
  display.setTextWrap(false);
...
void displayBLESpamDevice(const char* deviceName) {
  display.fillRect(0, 16, display.width(), 10, SSD1306_BLACK);
  ...
  if (inBadBLE) {
    if (deleteConfirm) {
      ...
      if (buttonOK.isClick()) {
        ...
        display.setTextWrap(false);
        if (success) {
          display.drawBitmap(5, 2, image_DolphinMafia_bits, 119, 62, SSD1306_WHITE);
