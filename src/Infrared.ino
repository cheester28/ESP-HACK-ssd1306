...
void displayIRSelection(byte menuIndex, String signalName = "") {
  ...
  if (menuIndex == 0 || menuIndex == 2 || menuIndex == 3 || menuIndex == 4) {
    display.drawBitmap(14, 12, image_Power_bits, 38, 40, SSD1306_WHITE);  // Изменено с SH110X_WHITE
  }
...
void drawSendingScreen(int progress) {
  ...
  if (irMenuIndex == 0 || irMenuIndex == 2 || irMenuIndex == 3 || irMenuIndex == 4) {
    display.drawBitmap(14, 12, image_Power_hvr_bits, 38, 40, SSD1306_WHITE);  // Изменено с SH110X_WHITE
    display.setTextSize(1);
    display.setCursor(62, 43);
...
void drawReadingScreen() {
  ...
  if (!readSignal) {
    ...
    display.drawBitmap(0, 12, image_InfraredLearnShort_bits, 128, 31, SSD1306_WHITE);  // Изменено
  } else {
...
void drawFileExplorer() {
  ...
  for (int i = 0; i < perPage; i++) {
    ...
    if (idx == irFileIndex) {
      display.fillRect(0, 24 + i * 10 - 1, display.width(), 10, SSD1306_WHITE);  // Изменено
      display.setTextColor(SSD1306_BLACK);  // Изменено с SH110X_BLACK
    } else {
      display.setTextColor(SSD1306_WHITE);  // Изменено
    }
